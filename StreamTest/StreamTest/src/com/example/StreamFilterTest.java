...
public class StreamFilterTest {
    ...
    public static void main(String[] args) {
        ...
        personList.stream().
		   					filter(p->p.getCity().equals(City.Tainan) || p.getCity().equals(City.Kaohsiung)).
		   					forEach(service::sendMessage);
                
        //宣告Predicte過濾高雄男性成員, 使用forEach()傳簡訊給過濾後成員
        System.out.println("傳簡訊給高雄男性成員");
        Predicate<Person> filterKouMale= p->p.getCity()==City.Kaohsiung && p.getGender()==Gender.MALE;
        for(Person p:personList) {
        	if(filterKouMale.test(p)) {
        ...
        personList.stream().
							filter(p->p.getCity()==City.Tainan).
